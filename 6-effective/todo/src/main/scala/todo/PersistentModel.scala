...
object PersistentModel extends Model:
  ...
  def delete(id: Id): Boolean =
    val currentTasks = tasks.toMap
    val removedTasks = currentTasks.removed(id)
    saveTasks(Tasks(removedTasks))
    if currentTasks.size != removedTasks.size then
      true
    else
      false
